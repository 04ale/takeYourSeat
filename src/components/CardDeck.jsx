...
const CardDeck = ({ movies, title }) => {
  ...
  const [animation, setAnimation] = useState(''); // 'like' or 'dislike'

  // Inicializa o baralho com os primeiros 7 filmes
  useEffect(() => {
    // .reverse() é para que o filme de índice 0 seja o topo da pilha
  ...
  const handleAction = (action) => {
    ...
    setAnimation(action); // Ativa a animação de 'like' ou 'dislike'

    setTimeout(() => {
      setDiscarded(prev => [deck[deck.length - 1], ...prev]);
      setDeck(prev => prev.slice(0, -1));
      setAnimation(''); // Reseta a animação
  ...
  return (
    <div className="container mx-auto mt-12 px-4 md:px-0">
      ...
      <div className="relative h-96 w-64 mx-auto">
        {deck.length > 0 ? (
          deck.map((movie, index) => {
            ...
            // Aplica a animação de saída na carta do topo
            ...
            return (
              <div
                ...
                style={{
                  transform: isTopCard ? 'none' : `scale(${1 - (deck.length - 1 - index) * 0.05}) translateY(${(deck.length - 1 - index) * -12}px)`,
                  zIndex: index,
                }}
              >
      ...
      <div className="flex justify-center items-center gap-8 mt-6">
        <button onClick={() => handleAction('dislike')} className="bg-white rounded-full p-4 shadow-xl text-red-500 hover:scale-110 transition-transform" aria-label="Dislike">
          <X size={32} />
        </button>
        <button onClick={handleUndo} className="bg-white rounded-full p-3 shadow-lg text-gray-500 hover:scale-110 transition-transform" aria-label="Undo" disabled={discarded.length === 0}>
          <Undo2 size={20} />
        </button>
        <button onClick={() => handleAction('like')} className="bg-white rounded-full p-4 shadow-xl text-green-500 hover:scale-110 transition-transform" aria-label="Like">
          <Heart size={32} />
        </button>
      </div>
    </div>
