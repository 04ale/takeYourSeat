...
// 1. Importe o useNavigate para redirecionar o usuário
...
const Header = () => {
  ...
  const [searchValue, setSearchValue] = useState(""); // 2. Estado para o valor da busca
  ...
  // 4. Função que lida com o envio da busca
  const handleSubmit = (e) => {
    e.preventDefault(); // Previne o recarregamento da página
    if (!searchValue.trim()) return; // Não faz nada se a busca estiver vazia
    
    navigate(`/search?q=${searchValue}`);
    setSearchValue(""); // Limpa o campo após a busca
  ...
  return (
    <header className="bg-rose-100 text-[#333] shadow-md sticky top-0 z-50">
      <div className="container mx-auto flex justify-between items-center p-4 gap-4">
        ...
        {/* 5. Barra de pesquisa mobile agora é um formulário funcional */}
        ...
        {/* 6. Navegação Desktop também é um formulário funcional */}
        <nav className="hidden md:flex items-center gap-6">
          ...
          <NavLink to="/wishlist" className="font-semibold hover:text-rose-500" style={({ isActive }) => isActive ? activeLinkStyle : undefined}>
            Lista de Desejos
          </NavLink>
          <NavLink to="/my-ratings" className="font-semibold hover:text-rose-500" style={({ isActive }) => isActive ? activeLinkStyle : undefined}>
            Minhas Avaliações
          </NavLink>
          <Link to="/profile">
      ...
      {isMenuOpen && (
        <div className="md:hidden bg-rose-100 pb-4 px-4 border-t border-rose-200">
          <nav className="flex flex-col items-center gap-4 pt-4">
            <NavLink to="/wishlist" className="font-semibold" style={({ isActive }) => isActive ? activeLinkStyle : undefined}>Lista de Desejos</NavLink>
            <NavLink to="/my-ratings" className="font-semibold" style={({ isActive }) => isActive ? activeLinkStyle : undefined}>Minhas Avaliações</NavLink>
            <NavLink to="/profile" className="font-semibold flex items-center gap-2"><User size={20} /> Perfil</NavLink>
          </nav>
        </div>
      )}
    </header>
